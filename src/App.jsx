...
import { MapPin, Mountain, Snowflake, Layers, Tractor, Users, ArrowDown, Building2, Droplets, Globe } from 'lucide-react';
 ...
 * Now supports an 'image' prop for background photos!
 */
const BentoCard = ({ children, className = "", delay = 0, image = null }) => {
      ...
      {image && (
        <>
          <img 
            src={image} 
            alt="Background" 
            className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:scale-110 transition-transform duration-700 ease-in-out"
          />
    ...
    <div className="min-h-screen bg-black text-white selection:bg-purple-500 selection:text-white font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[minmax(180px,auto)]">
          ...
          {/* 2. Formation / Glaciation (Uses Hydro/Ice image) */}
          <BentoCard className="md:col-span-1 lg:col-span-1 row-span-2" delay={100} image="/hydro.jpg">
            <div className="h-full flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-3 mb-6">
                   <div className="p-3 bg-cyan-500/80 rounded-2xl text-white backdrop-blur-md">
                    <Snowflake size={24} />
                   </div>
                </div>
                
                <h3 className="text-2xl font-bold text-white mb-4 drop-shadow-md">Shaped by Ice</h3>
                
                <p className="text-cyan-100 text-sm leading-relaxed drop-shadow-md font-medium">
                  Glaciation was the key process. Massive retreating glaciers created big ancient lakes.
                </p>
              </div>
              
              <div className="mt-4 p-4 rounded-xl bg-black/60 border border-white/10 backdrop-blur-sm">
                 <p className="text-xs text-gray-300">
                   "Glaciers left thick deposits of sand and clay, forming the present flat plains."
                 </p>
              </div>
            </div>
          </BentoCard>

          {/* 3. Geology (Uses Rock image) */}
          <BentoCard className="md:col-span-1 lg:col-span-1" delay={200} image="/rocks.png">
            <div className="flex flex-col h-full justify-between">
              <div className="flex justify-between items-start">
                 <div className="p-3 bg-amber-500/80 rounded-2xl text-white backdrop-blur-md">
                  <Layers size={24} />
                 </div>
              </div>
              <div>
                <h3 className="text-xl font-bold text-white mt-4 drop-shadow-md">Sedimentary Rock</h3>
                <p className="text-amber-100 text-xs mt-2 font-medium drop-shadow-md">
          ...
          {/* 4. Population (Uses Map image) */}
          <BentoCard className="md:col-span-3 lg:col-span-2 row-span-2 border-purple-500/30" delay={300} image="/population.png">
              <div className="h-full flex flex-col justify-center items-center text-center relative z-10">
                <div className="p-4 bg-purple-500/80 backdrop-blur-md rounded-full mb-6 text-white shadow-lg">
                  <Users size={40} />
                </div>
                <h2 className="text-6xl md:text-8xl font-black text-white tracking-tighter mb-2 drop-shadow-xl">
                  60%
                </h2>
                <h3 className="text-xl md:text-2xl font-medium text-purple-200 mb-6 drop-shadow-md">
                  of Canada's Population
                </h3>
                <div className="grid grid-cols-2 gap-4 text-left w-full max-w-md bg-black/60 p-6 rounded-2xl backdrop-blur-md border border-white/10">
                   <div>
                     <div className="flex items-center gap-2 mb-1 text-white font-bold">
                       <Building2 size={16} /> Cities
                     </div>
                     <p className="text-xs text-gray-300">Home to Toronto & Montreal thanks to flat terrain.</p>
                   </div>
                   <div>
                     <div className="flex items-center gap-2 mb-1 text-white font-bold">
                       <ArrowDown size={16} className="rotate-[-45deg]" /> History
                     </div>
                     <p className="text-xs text-gray-300">Settlers arrived for the fertile agricultural soil.</p>
                   </div>
                </div>
              </div>
          </BentoCard>
            ...
            <div className="h-full flex flex-col justify-between">
               ...
               <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    <span className="px-2 py-1 bg-white/5 rounded-md text-xs text-gray-300">Clay</span>
                    <span className="px-2 py-1 bg-white/5 rounded-md text-xs text-gray-300">Sand</span>
                  </div>
               </div>
            </div>
          </BentoCard>
             ...
             <div className="flex flex-col md:flex-row items-center gap-6 h-full">
                ...
                <div className="flex-grow">
                   ...
                   <p className="text-gray-300 text-sm leading-relaxed mb-4">
                     Excellent for farming due to rich, fertile soils and flat landscape. It contains some of the best farmland in all of Canada.
                   </p>
                   <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden">
                     <div className="h-full bg-green-500 w-[90%] rounded-full shadow-[0_0_10px_rgba(34,197,94,0.5)]" />
                   </div>
                   <div className="mt-2 text-right text-xs text-green-400 font-mono">Suitability: Excellent</div>
                </div>
             </div>
          </BentoCard>
